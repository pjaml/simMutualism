...
Sometimes a simulation ends because the =maxIterations= number is reached, rather than actually reaching a steady state. Here we write a function to tell us if a simulation ended because it reached the =maxIterations=.

#+begin_src matlab :tangle reachedMaxIterations.m
function maxIterationsReached = reachedMaxIterations(simMatFile)
